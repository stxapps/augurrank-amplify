...
import { HTTP, SCS, ERR_INVALID_ARGS, ERR_NOT_FOUND, ERR_VRF_SIG } from '@/types/const';
...
export const mergeTrades = (...trades) => {
  const bin = {
    updateDate: null,
  ...
  for (const trade of trades) {
    ...
    if (isNumber(trade.updateDate)) {
      if (!isNumber(bin.updateDate) || trade.updateDate > bin.updateDate) {
        bin.updateDate = trade.updateDate;
      }
    }
    if (isString(trade.pStatus)) {
      if (trade.pStatus === SCS) bin.pStatus.scs = trade.pStatus;
      else if (trade.pStatus !== PDG) bin.pStatus.updg = trade.pStatus;
    }
    if (isString(trade.cStatus)) {
      if (trade.cStatus === SCS) bin.cStatus.scs = trade.cStatus;
      else if (trade.cStatus !== PDG) bin.cStatus.updg = trade.cStatus;
    }

    newTrade = { ...newTrade, ...trade };
  }

  if (isNumber(bin.updateDate)) newTrade.updateDate = bin.updateDate;

  if (isString(bin.pStatus.scs)) newTrade.pStatus = bin.pStatus.scs;
  else if (isString(bin.pStatus.updg)) newTrade.pStatus = bin.pStatus.updg;

  if (isString(bin.cStatus.scs)) newTrade.cStatus = bin.cStatus.scs;
  else if (isString(bin.cStatus.updg)) newTrade.cStatus = bin.cStatus.updg;
