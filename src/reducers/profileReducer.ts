...
import { isObject, isString, mergeTrades } from '@/utils';
...
const profileReducer = (state = initialState, action) => produce(state, draft => {
  if (action.type === UPDATE_PROFILE) {
    ...
    if (isObject(dataMore) && Array.isArray(dataMore.trades)) {
      if (isObject(draft.data) && isObject(draft.data.trades)) {
        for (const trade of dataMore.trades) {
          draft.data.trades[trade.id] = mergeTrades(draft.data.trades[trade.id], trade);
        }
      }
    }
